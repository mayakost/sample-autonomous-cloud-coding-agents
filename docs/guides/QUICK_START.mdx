...
Install these in the order shown — mise provisions the language runtimes used by the rest of the toolchain.

- **mise** - task runner ([install guide](https://mise.jdx.dev/getting-started.html)). Install and activate this first: it provisions the Node.js version the rest of the toolchain depends on.
- **Node.js** v20 or later (Node 24 is the supported maximum — see CI matrix). Provided by mise via `mise install` in Step 1 — you don't install it separately.
- **AWS CDK CLI** - `npm install -g aws-cdk`. Installs into the Node version provisioned by mise, so this must be run after `mise install` in Step 1.
...
- **Docker** - for building the agent container image. On an **x86_64** host you also need QEMU/binfmt to build the arm64 (Graviton) image — see the caution in Step 3. Can be installed at any point — it has no ordering dependency on mise.
